...
//		state.printMap();
		...
		while (matches != null && matches.length>0){
			...
			int i = 1;
			while (i < matches.length && match.p == null){
				match = matches[i];
				i++;
			}
			from = match.from;
...
//				moved.printMap();
	 ...
	 */
	private ArrayList<Board> moveBoxInAllDirections(Board board,Coords inFrom) throws IllegalMoveException {
		Coords from = new Coords (inFrom.x, inFrom.y);
		ArrayList<Board> boards = new ArrayList<Board> ();
	...
	private Board moveBox(Coords inFrom, Board.Direction inTo, Board board) throws IllegalMoveException {
	...
	private Board moveBox(Coords inFrom, Board.Direction inTo, 
			Board board, boolean disregardBoxes, boolean ignoreTunnels) throws IllegalMoveException {
...
//			System.out.println ("guarateeddeadlocks");
			return null;
		}
